    ...
    >
      <div className="flex items-start mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-base font-semibold text-[var(--text-primary)] truncate mb-0.5">
            {agent.name}
          </h3>
      ...
      {agent.capabilities && agent.capabilities.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {agent.capabilities.slice(0, 3).map((cap) => (
            <span
              key={cap.name}
              className="px-2 py-0.5 bg-[var(--ai-primary-soft)] text-[var(--ai-primary)] text-[10px] font-medium rounded-md"
            >
              {cap.name}
            </span>
          ))}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-[var(--border-color)]">
        <div className="flex items-center space-x-4 text-xs text-[var(--text-muted)]">
          ...
          <span className="flex items-center">
            <svg
              className="w-3.5 h-3.5 mr-1.5 text-[var(--ai-warning)]"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
