...
describe("MessageService", () => {
  ...
  describe("editMessage", () => {
    ...
    it("should persist editedAt and edit history for valid edits", async () => {
      ...
      mockMessageRepository.save.mockImplementation(
  ...
  describe("getMessageHistoryBySeq", () => {
    ...
    it("should throw ForbiddenException for group history when user is not member", async () => {
      ...
      await expect(
        service.getMessageHistoryBySeq("user-1", "group-1", "group", {
          fromSeq: 0,
        }),
    ...
    it("should return seq history result with hasMore and nextSeq", async () => {
      ...
      const result = await service.getMessageHistoryBySeq(
        "user-1",
        "user-2",
        "single",
        {
      ...
      expect(
        mockSequenceService.getMissingSequencesWithMeta,
      ).toHaveBeenCalledWith("single:user-1:user-2", 11, 99);
      expect(
        mockPrometheusService.incrementMessageSeqGapScan,
      ).toHaveBeenCalledWith("single", "success");
      expect(
        mockPrometheusService.incrementMessageSeqGapTruncated,
      ).toHaveBeenCalledWith("single");
      expect(
        mockPrometheusService.observeMessageSeqGapScanDuration,
      ).toHaveBeenCalledWith("single", "success", expect.any(Number));
    });
  ...
  describe("ackConversationSeq", () => {
    it("should advance seq and return sync state for single conversation", async () => {
      ...
      expect(
        mockConversationService.getConversationSyncStateForUser,
      ).toHaveBeenCalledWith("user-1", "user-2", "single");
      ...
      expect(
        mockPrometheusService.observeMessageSeqAckPendingSeq,
      ).toHaveBeenCalledWith("single", "user", 10);
      expect(
        mockPrometheusService.observeMessageSeqAckDuration,
      ).toHaveBeenCalledWith("single", "user", "success", expect.any(Number));
    });
    ...
    it("should reject group ack when user is not a member", async () => {
      ...
      expect(
        mockPrometheusService.observeMessageSeqAckDuration,
      ).toHaveBeenCalledWith("group", "user", "failure", expect.any(Number));
    });
    ...
    it("should advance device cursor when deviceId is provided", async () => {
      ...
      expect(result.success).toBe(true);
      expect(
        mockConversationService.advanceDeviceLastReadSeq,
      ).toHaveBeenCalledWith("user-1", "ios-1", "user-2", "single", 120);
      expect(
        mockConversationService.getConversationSyncStateForUser,
      ).toHaveBeenCalledWith("user-1", "user-2", "single", {
        deviceId: "ios-1",
      ...
      expect(
        mockPrometheusService.observeMessageSeqAckPendingSeq,
      ).toHaveBeenCalledWith("single", "device", 30);
      expect(
        mockPrometheusService.observeMessageSeqAckDuration,
      ).toHaveBeenCalledWith("single", "device", "success", expect.any(Number));
    });

    it("should return success with null state when sync-state query fails after ack update", async () => {
      mockConversationService.advanceLastReadSeq.mockResolvedValue(true);
      mockConversationService.getConversationSyncStateForUser.mockRejectedValue(
  ...
  describe("ackConversationSeqBatch", () => {
    it("should deduplicate batch items and keep max ackSeq per conversation", async () => {
      ...
      expect(
        mockPrometheusService.incrementMessageSeqAckBatch,
      ).toHaveBeenCalledWith("user", "success");
      expect(
        mockPrometheusService.observeMessageSeqAckBatchFailedItems,
      ).toHaveBeenCalledWith("user", 0);
      expect(
        mockPrometheusService.observeMessageSeqAckBatchDuration,
      ).toHaveBeenCalledWith("user", "success", expect.any(Number));
    });
    ...
    it("should isolate per-item ack failures instead of failing entire batch", async () => {
      ...
      expect(
        mockPrometheusService.observeMessageSeqAckDuration,
      ).toHaveBeenCalledWith("single", "user", "failure", expect.any(Number));
      expect(
        mockPrometheusService.incrementMessageSeqAckBatch,
      ).toHaveBeenCalledWith("user", "partial");
      expect(
        mockPrometheusService.observeMessageSeqAckBatchFailedItems,
      ).toHaveBeenCalledWith("user", 1);
      expect(
        mockPrometheusService.observeMessageSeqAckBatchDuration,
      ).toHaveBeenCalledWith("user", "partial", expect.any(Number));
    });
  ...
  describe("group receipt member query guards", () => {
    ...
    it("getGroupMessageReadMembers should throw on non-group message", async () => {
      ...
      await expect(
        service.getGroupMessageReadMembers("user-1", "msg-1", {
          limit: 20,
          offset: 0,
        }),
      ).rejects.toBeInstanceOf(BadRequestException);
  ...
  describe("group receipt member cursor pagination", () => {
    ...
    it("getGroupMessageUnreadMembers should support cursor paging and return nextCursor", async () => {
      ...
      const result = await service.getGroupMessageUnreadMembers(
        "user-1",
        "msg-1",
        {
          limit: 2,
      ...
      const cursorConditionCall =
        queryBuilders.baseQueryBuilder.andWhere.mock.calls.find(
          (call) =>
            typeof call[0] === "string" && call[0].includes("cursorTime"),
        );
    ...
    it("getGroupMessageReadMembers should support cursor paging and return nextCursor", async () => {
      ...
      const result = await service.getGroupMessageReadMembers(
        "user-9",
        "msg-9",
        {
          limit: 2,
  ...
  describe("unique constraint recovery", () => {
    ...
    it("should fallback to per-message send when batch save hits unique constraint", async () => {
      ...
      const sendSpy = jest
        ...
        .mockResolvedValue(fallbackResult as any);
