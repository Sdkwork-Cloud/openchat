...
describe('MessageController', () => {
  function createController() {
    ...
    const controller = new MessageController(messageService as any, messageReactionService as any);
  ...
  it('should derive different clientSeq for each batch item from request idempotency key', async () => {
    ...
    messageService.sendMessageBatch.mockResolvedValue({
      success: true,
      results: [
        { success: true, message: { id: 'msg-1', uuid: 'uuid-1', status: MessageStatus.SENT } },
    ...
    expect(normalizedBatch[0].clientSeq).not.toEqual(normalizedBatch[1].clientSeq);
  ...
  it('should set a message reaction using the authenticated userId', async () => {
    const { controller, messageService, messageReactionService } = createController();
