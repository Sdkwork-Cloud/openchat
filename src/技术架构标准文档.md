...
# 数据库
npm run db:sync             # 同步实体到数据库（开发环境）
npm run db:drop             # 删除数据库结构（仅开发环境）
./scripts/init-database.sh dev  # 执行 schema.sql + seed.sql 初始化
psql -h <host> -U <user> -d <db_name> -f database/seed.sql  # 导入种子数据
