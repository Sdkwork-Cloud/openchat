...
function runPsqlQuery(config, databaseName, sql) {
  const result = runCommand(
    'psql',
    ['-h', config.host, '-p', String(config.port), '-U', config.user, '-d', databaseName, '-tA', '-c', sql],
    {
...
function detectDatabaseAction(projectRoot, options = {}) {
  if (!commandExists('psql')) {
    throw new Error('psql was not found in PATH. Install the PostgreSQL client before running deploy.');
  }
...
function renderSystemdUnit(options) {
  return [
    ...
    `EnvironmentFile=${options.projectRoot}/.env`,
    ...
    `PIDFile=${options.projectRoot}/var/run/openchat.pid`,
    'GuessMainPID=no',
    `ExecStart=${options.projectRoot}/bin/openchat start --host ${options.host} --port ${options.port} --environment ${options.environment}`,
    `ExecStop=${options.projectRoot}/bin/openchat stop`,
    `ExecReload=${options.projectRoot}/bin/openchat restart --host ${options.host} --port ${options.port} --environment ${options.environment}`,
    'Restart=on-failure',
    'RestartSec=5',
    '',
    '[Install]',
...
function installSystemdService(projectRoot, options) {
  if (process.platform !== 'linux') {
    logWarn('Systemd service installation is only supported on Linux. Skipping --service.');
    return 0;
  }
  ...
  const stagedUnitPath = path.join(projectRoot, 'var', 'data', 'openchat.service.generated');
  ...
  const systemctlArgs = shouldUseSudo
    ? ['sudo', 'systemctl']
    : ['systemctl'];
  ...
  if (!commandExists(systemctlArgs[0], systemctlArgs.length === 2 ? [systemctlArgs[1], '--version'] : ['--version'])) {
    throw new Error('systemctl is required to install the OpenChat service.');
  }

  runLocalCommand(copyArgs[0], copyArgs.slice(1));
  runLocalCommand(systemctlArgs[0], [...systemctlArgs.slice(1), 'daemon-reload']);
  runLocalCommand(systemctlArgs[0], [...systemctlArgs.slice(1), 'enable', 'openchat.service']);
  runLocalCommand(systemctlArgs[0], [...systemctlArgs.slice(1), 'restart', 'openchat.service']);
...
async function runDeploy(projectRoot, options = {}) {
  ...
  const port = Number.parseInt(String(options.port || process.env.PORT || '7200'), 10);
  ...
  const databaseAction = requestedDbAction === 'auto'
