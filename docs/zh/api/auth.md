...
认证授权 API 的路径前缀为 `/im/v3/auth`。

| 接口 | 方法 | 路径 | 说明 | 是否需要认证 |
|------|------|------|------|-------------|
| 用户登录 | POST | `/auth/login` | 用户登录获取令牌 | 否 |
| 用户登出 | POST | `/auth/logout` | 用户登出 | 是 |
| 获取设备会话列表 | GET | `/auth/devices` | 获取当前用户设备会话摘要 | 是 |
| 当前设备登出 | POST | `/auth/logout/device` | 登出当前设备并清理该设备会话游标 | 是 |
| 指定设备登出 | POST | `/auth/logout/device/:deviceId` | 登出指定设备并清理该设备会话游标 | 是 |
| 登出其它设备 | POST | `/auth/logout/others` | 保留当前设备，登出其它设备 | 是 |
...
| deviceId | string | 否 | 设备ID（建议传递，格式：`[A-Za-z0-9._:-]{1,64}`） |
...
- 如果 JWT 未绑定 `deviceId`，则不能在设备级接口中携带 `deviceId` 参数。
- 如果 JWT 已绑定 `deviceId`，请求中的 `deviceId`（若提供）必须与 JWT 中一致。
...
- 400: 设备ID格式无效
- 401: Token 无效或过期
- 403: 设备ID与认证设备不一致，或未绑定设备却请求设备级操作
...
```http
POST /im/v3/auth/forgot-password
Content-Type: application/json
```
...
```http
GET /im/v3/auth/users/:id/online-status
Authorization: Bearer &lt;access-token&gt;
```
...
```http
POST /im/v3/auth/users/online-status/batch
Authorization: Bearer &lt;access-token&gt;
Content-Type: application/json
```
