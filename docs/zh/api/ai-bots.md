...
所有 AI 机器人 API 都需要 JWT 认证，路径前缀为 `/im/v3/ai-bots`。
...
```http
POST /im/v3/ai-bots/:id/activate
Authorization: Bearer <access-token>
```
...
```http
POST /im/v3/ai-bots/:id/deactivate
Authorization: Bearer <access-token>
```
...
```http
POST /im/v3/ai-bots/:id/messages
Authorization: Bearer <access-token>
Content-Type: application/json
```
